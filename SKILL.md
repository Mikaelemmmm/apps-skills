...
name: apps-skills
description: |
  基于go-zero构建的企业级内部微服务基础框架，适用于单体应用和微服务架构，支持企业内部开箱即用。
...
本技能包提供基于go-zero的企业级内部微服务基础框架开发规范，专为 AI Agent 优化，帮助开发者构建生产级服务。覆盖内容：REST API、RPC 服务、数据库操作、系统韧性设计、问题排查、日志与错误码处理。
...
| **方案二** | HTTP（编排）+ gRPC（原子） | 中等规模、需要服务复用     | 中     |
| **方案三** | BFF + 领域服务层           | 大团队、多端接入、领域清晰 | 高     |
...
| 层级     | 方案二                                                             | 方案三                                                       |
| -------- | ------------------------------------------------------------------ | ------------------------------------------------------------ |
| **HTTP** | • 业务逻辑编排<br>• 调用多个 gRPC 完成业务流程<br>• 可包含业务规则 | • 无业务逻辑<br>• 聚合裁剪、透传写入<br>• Query 跨服务时聚合 |
